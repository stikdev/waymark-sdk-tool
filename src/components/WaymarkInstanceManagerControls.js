  ...
  return (
    <form
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await waymarkInstance.openEditorForTemplate(formElement.variantSlug.value);
    ...
    >
      ...
      <label htmlFor="variantSlug" className="form-label">
        Template ID (variant slug) -- to force this call to fail, pass in "test-reject"
      </label>
      <input
        type="text"
      ...
      />
      <button className="submit-button">Call waymark.openEditorForTemplate()</button>
  ...
  return (
    ...
    >
      ...
      <label htmlFor="videoID" className="form-label">
        Video ID (user video GUID) -- to force this call to fail, pass in "test-reject"
      </label>
      <input
        type="text"
        className="form-input"
      ...
      />
      <button className="submit-button">Call waymark.openEditorForVideo()</button>
      <OperationResults results={status} />
    </form>
  );
}
...
function GetTemplatesForCollectionTestForm({ waymarkInstance }) {
  ...
  return (
    <form
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          const newTemplates = await waymarkInstance.getTemplatesForCollection(collectionID);

          console.log(`Received templates for collection ${collectionID}:`, newTemplates);
    ...
    >
      ...
      />
      <button className="submit-button" data-test="getTemplatesForCollection-button">
  ...
  return (
    ...
    >
      ...
      <label className="form-label" htmlFor="createAccountPrivateKey">
        JWT Private Key -- the accepted test harness secret is "test-secret"; changing it to
        anything else will cause the call will be rejected
      </label>
      <input
        type="text"
        className="form-input"
      ...
      <label className="form-label" htmlFor="createAccountPartnerID">
        JWT Partner ID -- if this does not match the Partner ID used to initialize the Waymark
        instance, the call will be rejected
      </label>
      <input
        type="text"
        className="form-input"
      ...
      <label className="form-label">
        ...
        "existing-account@test.com" or `externalID` is set to "existing-external-id"
      </label>
      <JsonEditor
        value={accountData}
  ...
  return (
    ...
    >
      ...
      <label className="form-label" htmlFor="loginAccountPrivateKey">
        JWT Private Key -- the accepted test harness secret is "test-secret"; changing it to
        anything else will cause the call to fail
      </label>
      <input
        type="text"
        className="form-input"
...
function UpdateAccountInfoTestForm({ waymarkInstance }) {
  ...
  return (
    ...
    >
      ...
      <label className="form-label">
        Account data -- setting `firstName` to "test-reject" will force the call to fail
      </label>
      <JsonEditor
...
export default function WaymarkInstanceManagerControls({ waymarkInstance }) {
  useEffect(() => {
    waymarkInstance.on('editorExited', () => console.log('USER CLOSING EDITOR'));
