...
export default function GPUIntensiveTask() {
  ...
  useEffect(() => {
    ...
    if (shouldRunGPUIntensiveTask) {
      // Pixi code taken directly from Pixi.js example: https://pixijs.io/examples/#/mesh-and-shaders/multipass-shader-generated-mesh.js
      /* BEGIN PIXI APP SETUP */
      ...
      const perlinTexture = PIXI.Texture.from(perlinTextureImage);
      ...
      pixiStarWarpContainerRef.current.appendChild(app.view);
      app.start();
    }
  ...
  }, [shouldRunGPUIntensiveTask]);

  return (
    <>
      <label className="form-label">Should run GPU-intensive task</label>
      <input
        type="checkbox"
        checked={shouldRunGPUIntensiveTask}
        onChange={(event) => setShouldRunGPUIntensiveTask(event.target.checked)}
      />
      <div ref={pixiStarWarpContainerRef} />
    </>
  );
}
